...
test('badges distinguish recorded subscriptions from extra usage, never guessing a category from USD 200',()=>{
 ...
 for(const plan of ['', 'USD 200.00','200 USD on the 25th','Anthropic','Subscription plus prepaid credits','Credit card payment','Prepaid credit card','Tokenized card payment'])assert.equal(purchaseTypeLabel(plan),'待分类');
});
