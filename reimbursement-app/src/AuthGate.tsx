...
export default function AuthGate({children}:{children:ReactNode}) {
  ...
  return <main className="auth-page"><section className="auth-card"><div className="auth-mark">报</div><h1>报销工作台</h1><p>{!state?connecting?'正在连接服务器…':'未能连接服务器。':state.configured?'登录以查看和维护报销材料。':setupToken?'首次使用，请设置登录密码。':desktop?'首次使用，请粘贴一次性设置链接。':'工作台尚未设置密码。请使用管理员提供的一次性设置链接。'}</p>
    ...
    <footer><span className="auth-api-address">API：{apiAddress()||(connecting?'正在读取连接配置':'尚未读取连接配置')}</span><span>财务材料保存在服务器</span>{desktop&&<button type="button" className="connection-link" disabled={busy} onClick={()=>setSettingsOpen(true)}>服务器连接</button>}{desktop&&<UpdateCheck/>}</footer>
