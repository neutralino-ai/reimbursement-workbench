...
import './workflow.css';
import {vendorLabel, recordSourceCurrent} from './vendors';
...
export default function WorkflowView({ data, onOpenRecord, onReload }: Props) {
  ...
  const visible = rows.filter(({ record, currentStage }) => (filter === 'all' || (filter === 'completed' ? isFinanceCompleted(record) : filter === 'pending' ? !isFinanceCompleted(record) : currentStage === filter)) && `${record.billingMonth} ${record.date} ${record.invoiceNumber} ${record.accountName} ${record.accountID} ${record.vendor} ${record.plan} ${record.submissionReference}`.toLowerCase().includes(query));
