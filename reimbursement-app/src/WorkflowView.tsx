...
import {activeBatches,preparingBatches,batchListEntries,batchAmounts,batchTitle,lockedForBatch} from './claim-batches';
...
export default function WorkflowView({ data, onOpenRecord, onReload }: Props) {
  ...
  function renderFinishedBatch(batch:NonNullable<Workspace['claimBatches']>[number],mobile:boolean){
    ...
    const review=members.length===batch.recordIDs.length&&members.every(arePriorStepsComplete);
    const submitted=members.length===batch.recordIDs.length&&members.every(r=>getRecordWorkflow(r,data).find(s=>s.id==='submission')?.state==='done');
    const prepared=members.length===batch.recordIDs.length&&members.every(r=>applicationDocuments(r,data).some(d=>d.batchID===batch.id));
    const label=complete?'已报销':review?'财务审核中':submitted?'已交财务':members.some(r=>lockedForBatch(r,data))?'交付 / 财务进度待核对':prepared?'材料备妥，待交财务':'进度待核对';
    const document=(data.documents||[]).find(d=>d.batchID===batch.id&&d.status==='ready'&&!d.stale&&!d.needsUpdate);
    const pdf=document?.materials.find(m=>m.id===document.submissionPDFMaterialID);
    return <Fragment key={batch.id}><tr className={`ov-completed-batch${complete?' ov-reimbursed':''}`}><td colSpan={mobile?2:6}><div className="ov-batch-header"><strong>合并报销 · {batchTitle(members)}</strong><span>{totals.totalCNY?'¥'+totals.totalCNY:'人民币待核对'}</span><span>{label}</span><button className="button secondary small" aria-expanded={open} onClick={()=>setExpanded(ids=>ids.includes(batch.id)?ids.filter(id=>id!==batch.id):[...ids,batch.id])}>{open?'收起明细':`查看 ${members.length} 笔明细`}</button>{pdf&&<FileLink material={pdf}>报销说明 PDF</FileLink>}</div></td></tr>{open&&members.map(record=>mobile?<tr key={record.id}><td colSpan={2}><button className="ov-mobile-action" onClick={()=>onOpenRecord(record.id,mobileNextAction(record,data).step)}><strong>{record.billingMonth} · {record.invoiceNumber}</strong><span>{amount(record)} · {mobileNextAction(record,data).title}</span></button></td></tr>:renderRow(rows.find(row=>row.record.id===record.id)!))}</Fragment>;
  }
  ...
  return <section className="reimbursement-overview" aria-label="费用报销总览">
    ...
    <div className={`ov-batch-toolbar ${mobileSelecting?'is-selecting':''}`}><span>{chosen.length?`已选 ${chosen.length} 笔 · ${selectedAmounts.totalCNY?'¥'+selectedAmounts.totalCNY:'待补汇率'}`:'勾选 2–10 笔费用，合并准备材料'}</span><ActionButton className="button primary small" reason={chosen.length<2?'请先勾选至少 2 笔可合并的费用。':chosen.length>10?'一次最多合并 10 笔费用，请减少选择。':''} onClick={()=>{setEditor({recordIDs:chosen.map(r=>r.id)});setSelected([]);setMobileSelecting(false);}}>合并准备材料{chosen.length?`（${chosen.length}）`:''}</ActionButton>{chosen.length>0&&<button className="text-button" onClick={()=>setSelected([])}>取消选择</button>}</div>
