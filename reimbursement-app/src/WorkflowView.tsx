...
export default function WorkflowView({ data, onOpenRecord, onReload }: Props) {
  ...
  const visible = rows.filter(({ record }) => (filter === 'all' || (filter === 'completed' ? isFinanceCompleted(record) : !isFinanceCompleted(record))) && matchesProgress(record, data, progress) && `${record.billingMonth} ${record.date} ${record.invoiceNumber} ${record.accountName} ${record.submissionReference} ${vendorLabel(record.vendor)} ${record.vendor==='claude'?'anthropic':record.vendor==='chatgpt'?'openai gpt':''}`.toLowerCase().includes(query));
