...
function cellModel(record: RecordItem, id: WorkflowStepId, data: Workspace, sourceCurrent: boolean): CellModel {
  ...
  if (id === 'claim') {
    ...
    return { done: false, title, detail: missing ? `缺${missing}` : '缺情况说明及整合 PDF', ...(exchange.screenshots[0] ? { link: { material: exchange.screenshots[0], label: '查看汇率截图' } } : {}) };
  }
  if (id === 'submission') {
...
export default function WorkflowView({ data, onOpenRecord }: Props) {
