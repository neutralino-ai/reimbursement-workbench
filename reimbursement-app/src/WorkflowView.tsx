...
export default function WorkflowView({ data, onOpenRecord, onReload }: Props) {
  ...
  const rows = sorted.map(record => ({ record, cells: columns.map(column => cellModel(record, column.id, data, recordSourceCurrent(record, data, freshDays))) }));
  const visible = rows.filter(({ record }) => (filter === 'all' || (filter === 'completed' ? isFinanceCompleted(record) : !isFinanceCompleted(record))) && `${record.billingMonth} ${record.date} ${record.invoiceNumber} ${record.accountName} ${record.accountID} ${record.vendor} ${record.plan} ${record.submissionReference}`.toLowerCase().includes(query));
