...
export default function BatchEditor({data,recordIDs,batch,onReload,onClose,delivery}:{data:Workspace;recordIDs:string[];batch?:ClaimBatch;onReload:()=>Promise<unknown>;onClose:()=>void;delivery?:ReactNode}){
  ...
  return createPortal(<dialog ref={dialog} className="batch-dialog" aria-label="合并准备报销材料" onCancel={e=>{e.preventDefault();e.stopPropagation();close();}} onKeyDown={e=>e.stopPropagation()}>
    ...
    <div className="batch-content">
      ...
      <div className="ai-actions"><button className="button secondary" disabled={busy||uploadBusy||pending||!!generating||locked} onClick={()=>void run(async()=>{await save();setMessage('合并包已保存。');})}>保存共同说明</button><button className="button primary" disabled={busy||uploadBusy||pending||!!generating||locked||!canGenerate} onClick={()=>void run(async()=>{const current=await save();await post('batch-packet',{id,baseVersion:current.version,recordVersions:Object.fromEntries(records.map(r=>[r.id,r.version])),totalCNY:amounts.totalCNY,confirmed:true});const next=await api<Workspace>('/api/workspace');const updated=next.claimBatches?.find(b=>b.id===id);if(updated)remember(updated);setMessage('已确认逐笔金额，正在生成一份整合 PDF。');})}>{generating?'正在生成…':'确认逐笔金额，生成合并 PDF'}</button></div>
