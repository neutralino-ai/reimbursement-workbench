...
function ApplicationFiles({data, record}: {data: Workspace; record: RecordItem}) {
  ...
  const task = `请按 reimbursement-workflow Skill 处理 ${record.billingMonth} 的 ${record.invoiceNumber}。先通过 MCP 读取记录 ${record.id} 的当前版本，以及 workspace.policies 中的当前规则原件、条款与适用范围。${policyBasis ? `当前依据：${policyBasis}。` : ''}用户已确认：有适用明文按条款，没有条款沿用过去成功报销的同类先例。${record.vendor === 'claude' ? 'Claude' : 'GPT'} 采用 invoice 日期 ${record.date} 的中国银行 ${record.currency} 中行折算价（每100外币），无需再次确认汇率类别；说明中标注“沿用已通过报销先例”，不写成制度明文。保存官网实际截图并用 exchangeRate.set 登记。先沿用本条已有发票和付款原件，缺失时检查旧 DOCX/PDF；不得借用其他月份证据。由 Agent 填写情况说明，生成可编辑 DOCX，将说明、付款截图、科研用途截图和汇率截图整合为报销说明 PDF，不嵌入发票页面。ZIP 单独包含报销说明 PDF 和发票原件。用 document.register 的 purpose:application、submissionFormat:separate-invoices-v1 和 submissionPDFMaterialID 登记来源、精确版本和渲染检查结果；sourceMaterialIDs 仍包含发票原件和所引用的制度原件，说明中注明制度版本、条款及页码。缺原件或金额尚未核实的申请包保留草稿，不因制度未明确汇率类别而阻塞。已提交或获批金额不得因本次汇率重算自动覆盖。${record.vendor === 'chatgpt' && record.billingMonth === '2026-06' ? '六月提交归属未知仍需核对。' : ''}已进入财务审核的记录无需重建申请。`;
  ...
  return <section className="drawer-section application-section">
    ...
    {submissionPDFs.map(material => <AuthenticatedFileLink key={material.id} className="application-pdf" href={material.href} filename={material.filename}><span><strong>报销说明 PDF</strong><small>{ready.find(document=>document.submissionPDFMaterialID===material.id)?.submissionFormat==='separate-invoices-v1'?'含付款、汇率及科研用途截图；发票原件单独随 ZIP 提交':'历史版本；按新格式打包前需重新生成'}</small></span><span>查看 PDF</span></AuthenticatedFileLink>)}
    {!submissionPDFs.length && !priorComplete && <div className="application-missing">报销说明 PDF 待生成。集齐凭证、确认金额后，整合说明、付款及汇率等截图，发票原件单独放入 ZIP。</div>}
