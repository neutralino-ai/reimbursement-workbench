...
export type Job={id:string;kind:string;recordID?:string;batchID?:string;recordIDs?:string[];sourceBatchVersion?:string;status:string;createdAt:string;finishedAt?:string;error?:string;current?:boolean;result?:{submissionFormat?:string;purpose?:string;missing?:string[];reasons?:string[];documentID?:string;pdfMaterialID?:string;docxMaterialID?:string;materialID?:string;approved?:boolean;pages?:number;invoiceCheck?:{checks:{field:string;result:string}[]};paymentCheck?:{checks:{field:string;result:string}[]}}};
...
function ZipPanel({data,onReload}:{data:Workspace;onReload:()=>Promise<Workspace>}){
  ...
  const readyDocs=(data.documents||[]).filter(d=>d.purpose==='application'&&d.status==='ready'&&!d.stale&&!d.needsUpdate);
  const docs=readyDocs.filter(d=>d.submissionFormat==='separate-invoices-v1');
  const hasLegacy=readyDocs.some(d=>d.submissionFormat!=='separate-invoices-v1'&&d.recordIDs.some(id=>{const r=data.records.find(r=>r.id===id);return r&&!r.financeReviewPending&&r.status!=='completed';}));
